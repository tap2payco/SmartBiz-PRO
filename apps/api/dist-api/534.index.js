...
;// CONCATENATED MODULE: ../../node_modules/hono/dist/request/constants.js
...
// src/router.ts
var router_METHOD_NAME_ALL = "ALL";
...
var Hono = class _Hono {
  ...
  */
  ...
  constructor(options = {}) {
    ...
    this.use = (arg1, ...handlers) => {
      ...
      handlers.forEach((handler) => {
        this.#addRoute(router_METHOD_NAME_ALL, this.#path, handler);
      });
      return this;
    };
  ...
  mount(path, applicationHandler, options) {
    ...
    this.#addRoute(router_METHOD_NAME_ALL, mergePath(path, "*"), handler);
    return this;
  }
...
;// CONCATENATED MODULE: ../../node_modules/hono/dist/router/reg-exp-router/matcher.js
// src/router/reg-exp-router/matcher.ts

var matcher_emptyParam = [];
function matcher_match(method, path) {
  const matchers = this.buildAllMatchers();
  const match2 = ((method2, path2) => {
    const matcher = matchers[method2] || matchers[router_METHOD_NAME_ALL];
...
function buildMatcherFromPreprocessedRoutes(routes) {
  ...
  for (let i = 0, j = -1, len = routesWithStaticPathFlag.length; i < len; i++) {
    ...
    if (pathErrorCheckOnly) {
      staticMap[path] = [handlers.map(([h]) => [h, /* @__PURE__ */ Object.create(null)]), matcher_emptyParam];
    } else {
      j++;
    }
    let paramAssoc;
...
var router_RegExpRouter = class {
  name = "RegExpRouter";
  #middleware;
  #routes;
  constructor() {
    this.#middleware = { [router_METHOD_NAME_ALL]: /* @__PURE__ */ Object.create(null) };
    this.#routes = { [router_METHOD_NAME_ALL]: /* @__PURE__ */ Object.create(null) };
  }
  add(method, path, handler) {
    ...
    if (!middleware[method]) {
      ...
      [middleware, routes].forEach((handlerMap) => {
        ...
        Object.keys(handlerMap[router_METHOD_NAME_ALL]).forEach((p) => {
          handlerMap[method][p] = [...handlerMap[router_METHOD_NAME_ALL][p]];
        });
      });
    }
    if (path === "/*") {
    ...
    if (/\*$/.test(path)) {
      ...
      if (method === router_METHOD_NAME_ALL) {
        Object.keys(middleware).forEach((m) => {
          middleware[m][path] ||= findMiddleware(middleware[m], path) || findMiddleware(middleware[router_METHOD_NAME_ALL], path) || [];
        });
      } else {
        middleware[method][path] ||= findMiddleware(middleware[method], path) || findMiddleware(middleware[router_METHOD_NAME_ALL], path) || [];
      }
      Object.keys(middleware).forEach((m) => {
        if (method === router_METHOD_NAME_ALL || method === m) {
    ...
    for (let i = 0, len = paths.length; i < len; i++) {
      ...
      Object.keys(routes).forEach((m) => {
        if (method === router_METHOD_NAME_ALL || method === m) {
          routes[m][path2] ||= [
            ...findMiddleware(middleware[m], path2) || findMiddleware(middleware[router_METHOD_NAME_ALL], path2) || []
          ];
  ...
  #buildMatcher(method) {
    const routes = [];
    let hasOwnRoute = method === router_METHOD_NAME_ALL;
    [this.#middleware, this.#routes].forEach((r) => {
      const ownRoute = r[method] ? Object.keys(r[method]).map((path) => [path, r[method][path]]) : [];
      if (ownRoute.length !== 0) {
        hasOwnRoute ||= true;
        routes.push(...ownRoute);
      } else if (method !== router_METHOD_NAME_ALL) {
        routes.push(
          ...Object.keys(r[router_METHOD_NAME_ALL]).map((path) => [path, r[router_METHOD_NAME_ALL][path]])
        );
      }
    });
    if (!hasOwnRoute) {
...
;// CONCATENATED MODULE: ../../node_modules/hono/dist/router/reg-exp-router/prepared-router.js
...
var node_Node = class _Node {
  ...
  #getHandlerSets(node, method, nodeParams, params) {
    ...
    for (let i = 0, len = node.#methods.length; i < len; i++) {
      ...
      const handlerSet = m[method] || m[router_METHOD_NAME_ALL];
      const processedSet = {};
...
var hono_Hono = class extends Hono {
  ...
  constructor(options = {}) {
    ...
    this.router = options.router ?? new SmartRouter({
      routers: [new router_RegExpRouter(), new TrieRouter()]
    });
  }
};
